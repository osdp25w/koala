...
class RBACViewSetMixin:
    ...
    def get_rbac_model_class(self):
        """獲取要檢查權限的 Model 類"""
        # 避免遞迴調用 get_queryset()，直接使用 queryset 屬性
        if hasattr(self, 'queryset') and self.queryset is not None:
            return self.queryset.model
        # 回退方案：調用父類的 get_queryset 但不經過 RBAC 過濾
    ...
    def filter_request_data(self, data: Dict) -> Dict:
        """過濾 request 資料，只保留有權限的欄位"""
        if not self.RBAC_AUTO_FILTER_FIELDS:
            return data
        ...
        for field, value in data.items():
            if field in allowed_fields:
                ...
                logger.debug(
                    f"Filtered out field '{field}' for user {self.request.user.username}"
                )
    ...
    def _apply_field_filtering(self, data, field_access_levels, profile):
        ...
        # 創建一個假的實例對象來重用 RBACSerializerMixin 的邏輯
        mock_instance = type(
            'MockInstance',
            (),
            {
                'id': data.get('id'),
                'type': data.get('type'),
                'user': type('MockUser', (), {'id': data.get('user_id')})()
                if data.get('user_id')
                else None,
            },
        )()
