...
class StaffViewSetTest(BaseAPITestWithFixtures):
    ...
    def test_staff_update_password(self):
        """測試更新 Staff 密碼"""
        self.client.credentials(HTTP_AUTHORIZATION=self._get_auth_header(self.staff1))

        # 密碼需要加密傳遞
        from account.services.encryption import StaffEncryptionService

        encrypted_password = StaffEncryptionService.encrypt_data('newpassword123')
        ...
        # 驗證密碼是否正確更新（檢查能否用新密碼登入）
        self.staff1.user.refresh_from_db()
        self.assertTrue(self.staff1.user.check_password('newpassword123'))
